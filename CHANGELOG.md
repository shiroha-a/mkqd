...
- `executor/httpexec`: the `http` executor, which forwards each job to a
  ...
  URL. Both map response status onto retry / permanent failure, cap the
  response read, and refuse to follow redirects.
- `internal/safedial`: a dialer that rejects destinations outside the
  ...
  to so a re-resolving hostname cannot slip past. On by default for
  `webhook`, off by default for `http`.
